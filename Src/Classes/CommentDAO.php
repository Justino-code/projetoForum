...
abstract class CommentDAO	 extends Conexao{
	...
	protected function setCommentId($id_post){
		$sql = "SELECT id_com FROM user_accounts WHERE  = :id_post";
		$param = [':email'=>$email];

		$this->consulta($sql,$param);
		if($this->getResult()){
			$this->id_comment = call_user_func_array('array_merge',$this->getResult());
		}
	}
}
